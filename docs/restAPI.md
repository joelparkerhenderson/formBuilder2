...
- Clinical form type path values are restricted to `waiting_list_card`, `waiting_list_cards`, `operation_note`, `operation_notes`, `outpatient_outcome`, `outpatient_outcomes`, `treatment_summary`, and `treatment_summaries`.
...
Case note tracker currently uses a browser-local simulated backend so it can be reviewed without changing SWAS Java. The simulated store includes user preferences, selected batch UUID session state, and My picklist `cntPickList` rows for selected/received volumes. For localhost SWAS / real database review, `scripts/seed-cnt-real-db.ts` creates and reseeds `cnt_*` PostgreSQL tables from the same fictitious store shape and upserts CNT patients into the main `patients` table. The intended future server-backed REST contract is documented separately in `docs/cnt.restAPI.md`. A SWAS implementation would be rooted at `/api/cnt` in the Node reference server and `/formBuilder2/api/cnt` in SWAS/static deployment.
...
Returns latest `forms_index_current` rows for a patient, newest first.
...
Returns the latest version of one clinical form from the mapped form table.
...
Returns one exact historical version of a clinical form.

`GET /api/forms-index/:uuid/history`

Returns saved versions for one clinical form UUID, newest first, with `form_version`, `event_datetime`, `document_datetime`, and `saved_by`.

`POST /api/forms/:formType`

Inserts one clinical form row into the mapped form table. The body includes `uuid`, `version`, `patient_uuid`, `event_datetime`, `form_status`, `form_data`, and form-specific optional fields.
...
Inserts one `forms_index` row for a saved clinical form.
