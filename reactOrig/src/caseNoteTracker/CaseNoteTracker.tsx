...
type CntBookmarkRoute =
  ...
  | { kind: 'view'; view: View; clearHash: boolean };

function safeCntDirectView(value: string): View | null {
...
function readCntRequestedView(): View | null {
  if (typeof window === 'undefined') return null;
  return safeCntDirectView(new URLSearchParams(window.location.search).get('view') || '');
}

function readCntBookmarkRoute(): CntBookmarkRoute | null {
  if (typeof window === 'undefined') return null;
  const requestedView = readCntRequestedView();
  if (requestedView) return requestedView === 'home' ? { kind: 'home', clearHash: false } : { kind: 'view', view: requestedView, clearHash: false };
  ...
  const directView = safeCntDirectView(route);
  if (directView) return directView === 'home' ? { kind: 'home', clearHash: false } : { kind: 'view', view: directView, clearHash: false };
  if (route === 'locator') {
...
export default function CaseNoteTracker({ inline = false, initialPatientUuid = '', initialPatient, onBack }: CaseNoteTrackerProps = {}) {
  ...
  const requestedInitialView = React.useMemo(() => readCntRequestedView(), []);
  ...
  React.useEffect(() => {
    ...
    if (!bookmarkRouteAppliedRef.current) {
      ...
      if (bookmarkRoute?.kind === 'view') {
        setView(bookmarkRoute.view);
        return;
      }
  ...
  const handleLogin = (uuidValue: string, switchToHome = true, targetView: View = 'home') => {
  ...
  if (!user) {
    return (
      <Shell
        title={{ kicker: 'NHS Wales - Case note tracker', title: 'Login' }}
        footer={<FbButton variant="primary" onClick={goBack}>Back</FbButton>}
      >
            ...
            <div style={styles.loginGrid}>
              {store.users.map((candidate, index) => (
                <button
                  ...
                  onClick={() => handleLogin(candidate.uuid, !requestedInitialView, requestedInitialView || 'home')}
                >
...
function isUserSpecificView(view: View) {
  return ['batch', 'requests', 'request', 'inbox', 'returnList', 'clinics', 'myClinics', 'preferences'].includes(view);
}
...
function userBatchUuids(store: CntStore, userUuid: string) {
  ...
  if (configured) return configured.filter((batchUuid) => store.batches.some((batch) => batch.uuid === batchUuid));
