...
function nadexIdFromLine(line: string) {
  ...
  if (bracketedId) return bracketedId[1];
  return '';
}
...
export const fbMSISelector: React.FC<MSISelectorProps> = ({ 
  ...
  const handleSelect = (line: string) => {
    ...
    setSearchTerm(line);
    onChange(line, true, nadexIdFromLine(line));
    setShowDropdown(false);
    setHighlightedIndex(-1);
  ...
  if ((required || requiredForAudit) && !hasLabel) {
    if (!renderRequiredMarkers) return renderWithStandaloneValueError(renderContent());
