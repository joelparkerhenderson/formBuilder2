...
import { fbReadOnlyCodedIcon as FbReadOnlyCodedIcon, fbReadOnly as FbReadOnly, fbReadOnlyTableCell as FbReadOnlyTableCell } from './components/fbReadOnly';
...
export function WaitingListCardRoV(props: WaitingListCardRoVProps) {
  ...
  const renderField = (label: string, value: any, lookupTable?: Record<string, string>, units?: string, coded?: boolean) => {
    return <FbReadOnly label={label} value={value} lookupTable={lookupTable} units={units} coded={coded} />;
  };
                    ...
                    <FbTableBody>
                      {procedures.filter(p => p.procedure).map((proc) => (
                        <FbTableRow key={proc.id}>
                          ...
                          <FbReadOnlyTableCell>
                            <div className="fb-read-only-value-inline">
                              <span>{proc.procedure}</span>
                              {proc.procedure_coded !== undefined && <FbReadOnlyCodedIcon coded={proc.procedure_coded} />}
                            </div>
                            {proc.additionalInfo && (
                              <div style={{
                                fontSize: '0.8rem',
