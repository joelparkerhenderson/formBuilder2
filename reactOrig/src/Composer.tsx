...
type ActionModalState =
  | { kind: 'palette'; title: string; action: AddAction; parentId: string; types: DesignerComponentType[]; tableCell?: false }
  | { kind: 'palette'; title: string; action: AddAction; parentId: string; types: DesignerComponentType[]; tableCell: true }
...
const makeComponent = (type: DesignerComponentType, existing: DesignerComponentSpec[]): DesignerComponentSpec => {
  ...
  const label =
    ...
    type === 'fbSubqForOption' ? `Option subquestion ${sameTypeCount}` :
    type === 'fbInverseSubq' ? `Inverse subquestion ${sameTypeCount}` :
    type === 'fbNotificationTypeGroup' ? 'Notification type' :
    type === 'fbDateHeightWeightBMIRow' ? 'Date recorded / height / weight / BMI' :
  ...
  return {
    ...
    placeholder: ['fbTextInput', 'fbTime', 'fbTextArea', 'fbSmartDropdown'].includes(type) ? '' : undefined,
    options: ['fbDropdown', 'fbSmartDropdown', 'fbRadio', 'fbCheck'].includes(type)
    ...
    fullWidth: ['fbTextArea', 'fbSmartDropdown'].includes(type) ? false : undefined,
    noWidthConstraint: type === 'fbSmartDropdown' ? false : undefined,
    units: ['fbNumberInputWithUnits', 'fbReadOnly'].includes(type) ? 'units' : undefined,
    optionValue: type === 'fbSubqForOption' ? 'option1' : undefined,
    children: ['fbSection', 'fbGridRow', 'fbGridCell', 'fbGroup', 'fbInverseSubq', 'fbSubqForOption'].includes(type) ? [] : undefined,
  };
};
...
const getAllowedAddTypes = (parentType?: DesignerComponentType): DesignerComponentType[] => {
  ...
  if (parentType === 'fbRadio' || parentType === 'fbCheck') return [...questionTypes, 'fbInverseSubq'];
  if (parentType === 'fbDropdown' || parentType === 'fbSmartDropdown') return ['fbSubqForOption'];
  if (parentType === 'fbInverseSubq' || parentType === 'fbSubqForOption') return questionTypes;
...
export default function Composer() {
  ...
  const selectionModalKey = `${selectedId}|${selectedSeparator?.id || ''}|${selectedTableTarget ? `${selectedTableTarget.kind}:${selectedTableTarget.tableId}:${'columnIndex' in selectedTableTarget ? selectedTableTarget.columnIndex : ''}:${'rowIndex' in selectedTableTarget ? selectedTableTarget.rowIndex : ''}:${'separatorIndex' in selectedTableTarget ? selectedTableTarget.separatorIndex : ''}` : ''}`;
  const previousSelectionModalKeyRef = React.useRef(selectionModalKey);
  ...
  React.useEffect(() => {
    if (previousSelectionModalKeyRef.current !== selectionModalKey) {
      previousSelectionModalKeyRef.current = selectionModalKey;
      setActionModal(null);
    }
  }, [selectionModalKey]);

  const componentHasPreviewValue = React.useCallback((component: DesignerComponentSpec) => {
    if (component.type === 'fbDateHeightWeightBMIRow') {
      return Boolean(
        previewValues[`${component.id}-dateRecorded`] ||
        previewValues[`${component.id}-heightCm`] ||
        previewValues[`${component.id}-weightKg`]
      );
    }
  ...
  const incompleteRequiredCount = React.useCallback((component: DesignerComponentSpec): number => {
    const structuralTypes = new Set<DesignerComponentType>(['fbSection', 'fbGridRow', 'fbGridCell', 'fbGroup', 'fbSubqForOption', 'fbInverseSubq', 'fbTable']);
    const ownCount = component.required && !structuralTypes.has(component.type) && !componentHasPreviewValue(component) ? 1 : 0;
    return ownCount + (component.children || []).reduce((total, child) => total + incompleteRequiredCount(child), 0);
  }, [componentHasPreviewValue]);
  ...
  const setPreviewValueWithCoding = (id: string, value: string, coded?: boolean, nadexId?: string) => {
    setPreviewValues((current) => ({
      ...current,
      [id]: value,
  ...
  const isSubquestionDescendant = (component: DesignerComponentSpec) =>
    (findComponentPath(displayedDesign?.components || [], component.id) || []).some((part) =>
      part.id !== component.id && (part.type === 'fbRadio' || part.type === 'fbCheck' || part.type === 'fbInverseSubq' || part.type === 'fbSubqForOption')
    );

  const componentTextWeight = (component: DesignerComponentSpec, defaultWeight: React.CSSProperties['fontWeight'] = 500) =>
    component.boldOverride ? 500 : component.plainOverride || isSubquestionDescendant(component) ? 300 : defaultWeight;

  const componentLabelIsSubfield = (component: DesignerComponentSpec) =>
    !component.boldOverride && (component.plainOverride || isSubquestionDescendant(component));
  ...
  const isQuestionDataVisibleInRoV = (component: DesignerComponentSpec) => {
    ...
    if (component.type === 'fbDateHeightWeightBMIRow') {
      return Boolean(
        componentStringValue({ ...component, id: `${component.id}-dateRecorded` }).trim() ||
        componentStringValue({ ...component, id: `${component.id}-heightCm` }).trim() ||
        componentStringValue({ ...component, id: `${component.id}-weightKg` }).trim()
      );
    }
    if (component.type === 'fbReadOnly') return Boolean(String(component.defaultValue || '').trim() || stringValue.trim());
  ...
  const componentHasRoVData = (component: DesignerComponentSpec): boolean => {
    ...
    if (['fbSection', 'fbGridRow', 'fbGridCell', 'fbGroup', 'fbInverseSubq', 'fbSubqForOption'].includes(component.type)) {
  ...
  const renderSelectionChrome = (component: DesignerComponentSpec, children: React.ReactNode, options: SelectionChromeOptions = {}) => {
    ...
    return (
      <div
        ...
        style={{
          position: 'relative',
          border: (showSelectedComponentChrome || showAllPurpleBoxes) ? `0.1rem solid ${purple}` : options.compact ? 0 : '0.1rem solid transparent',
          borderRadius: 0,
  ...
  const renderQuestion = (
    ...
    if (readOnlyPreview && component.type === 'fbDateHeightWeightBMIRow') {
      return (
        <FbDateHeightWeightBMIRow
          ...
          onDateRecordedChange={(nextValue) => setPreviewValue(`${component.id}-dateRecorded`, nextValue)}
          onHeightCmChange={(nextValue) => setPreviewValue(`${component.id}-heightCm`, nextValue)}
          onWeightKgChange={(nextValue) => setPreviewValue(`${component.id}-weightKg`, nextValue)}
        />
      );
    }
    if (readOnlyPreview && component.type === 'fbReadOnly') {
      return (
        <FbReadOnly
          label={component.label}
          value={component.defaultValue || componentStringValue(component)}
    ...
    if (readOnlyPreview) {
      ...
      return (
        <FbReadOnly
          label={component.type === 'fbRadio' || component.type === 'fbCheck' ? '' : component.label}
          value={component.type === 'fbNumberInputWithUnits' ? stringValue : displayValue}
          units={component.type === 'fbNumberInputWithUnits' ? component.units || 'units' : undefined}
          coded={coded}
          bigLabel={!!component.bigLabel}
          boldLabel={!!component.boldLabel}
          preserveGridSpace
    ...
    if (component.type === 'fbTextInput') {
      return <div className="fb-question-container">{renderEditableLabel(component)}<FbTextInput id={component.id} name={component.id} required={inputRequired} requiredForAudit={!hasLabel && component.requiredForAudit} showRequiredMarkers={!hasLabel} value={value} placeholder={component.placeholder || ''} subfield={componentLabelIsSubfield(component)} onChange={(nextValue) => setPreviewValue(component.id, nextValue)} {...controlTooltipProps(component)} /></div>;
    }
    if (component.type === 'fbTime') {
      return <div className="fb-question-container">{renderEditableLabel(component)}<FbTime id={component.id} name={component.id} required={inputRequired} requiredForAudit={!hasLabel && component.requiredForAudit} value={value} placeholder={component.placeholder || ''} subfield={componentLabelIsSubfield(component)} onChange={(nextValue) => setPreviewValue(component.id, nextValue)} {...controlTooltipProps(component)} /></div>;
    }
    if (component.type === 'fbTextArea') {
      return <div className="fb-question-container">{renderEditableLabel(component)}<FbTextArea id={component.id} name={component.id} required={inputRequired} requiredForAudit={!hasLabel && component.requiredForAudit} value={value} placeholder={component.placeholder || ''} fullWidth={!!component.fullWidth} subfield={componentLabelIsSubfield(component)} onChange={(nextValue) => setPreviewValue(component.id, nextValue)} {...controlTooltipProps(component)} /></div>;
    }
    if (component.type === 'fbReadOnly') {
      return (
        <FbReadOnly
          label={component.label}
          value={component.defaultValue || componentStringValue(component) || 'Read only value'}
          units={component.units}
          preserveGridSpace
          bigLabel={!!component.bigLabel}
          boldLabel={!!component.boldLabel}
    ...
    if (component.type === 'fbSmartDropdown') {
      return (
        <div className="fb-question-container">
          ...
          />
          {(component.children || []).filter((child) => child.type !== 'fbSubqForOption' || child.optionValue === value || selectedId === child.id).map((child) => renderDesignedComponent(child))}
        </div>
      );
    }
    if (component.type === 'fbNumberInput') return <div className="fb-question-container">{renderEditableLabel(component)}<FbNumberInput id={component.id} name={component.id} required={inputRequired} requiredForAudit={!hasLabel && component.requiredForAudit} value={value} placeholder={component.placeholder || ''} subfield={componentLabelIsSubfield(component)} onChange={(nextValue) => setPreviewValue(component.id, nextValue)} {...controlTooltipProps(component)} /></div>;
    ...
    if (component.type === 'fbNotificationTypeGroup') {
      return (
        <FbNotificationTypeGroup
          name={component.id}
          value={value || component.defaultValue || 'inpatient-ed-non-specialist'}
          onChange={(nextValue) => setPreviewValue(component.id, nextValue)}
          subfield={componentLabelIsSubfield(component)}
        />
      );
    ...
    return (
      <div
        ...
        style={{
          ...
          height: isFullSize ? fbSeparatorThickness : isTall ? 'auto' : fbSeparatorThickness,
          minHeight: isFullSize ? fbSeparatorThickness : isTall ? '2rem' : fbSeparatorThickness,
          marginBottom: isTall && !isFullSize ? '0.35rem' : undefined,
          border: 'none',
  ...
  const renderGroupOption = (component: DesignerComponentSpec, group: DesignerComponentSpec) => {
    ...
    if (readOnlyPreview) {
      const visibleNormalChildren = normalChildren.filter(componentHasRoVData);
      return renderSelectionChrome(component, (
        <div style={{ margin: 0 }}>
          <div
            style={{ display: 'flex', alignItems: 'flex-start', gap: '0.35rem', fontWeight: componentTextWeight(component, 500), marginLeft: '0.4rem', whiteSpace: 'pre-line', lineHeight: 1.2 }}
            {...controlTooltipProps(component, true)}
          >
            <span aria-hidden="true" style={{ flex: '0 0 auto' }}>•</span>
            <span>{component.label}</span>
          </div>
          {visibleNormalChildren.length > 0 && (
            <div className="pl-6">
              {visibleNormalChildren.map((child) => renderDesignedComponent(child))}
            </div>
          )}
        </div>
      ), { compact: true });
    ...
    return renderSelectionChrome(component, (
      <div className="fb-subquestion-wrapper">
        ...
        {checked && normalChildren.length > 0 && (
          <div className="pl-6 pb-1">
            {normalChildren.map((child) => renderDesignedComponent(child))}
          </div>
        )}
        {!checked && inverseChildren.length > 0 && (
          <div className="pb-1">
            {inverseChildren.map((child) => renderDesignedComponent(child))}
          </div>
        )}
      </div>
    ), { compact: true });
  ...
  const renderDesignedComponent = (component: DesignerComponentSpec): React.ReactNode => {
    ...
    if (component.type === 'fbInverseSubq') {
      return renderSelectionChrome(component, (
        <div className="fb-subquestion-wrapper">
          <div className="fb-subquestion" style={{ paddingLeft: 0 }}>
            {renderEditableLabel(component)}
            {renderStackedChildren(component.id, component.children || [])}
          </div>
        </div>
      ), { compact: true });
    }
    if (component.type === 'fbSubqForOption') {
      return renderSelectionChrome(component, (
        <div className="fb-subquestion-wrapper">
        ...
        <FbGroup
          ...
          label={(
            <span
              ...
              suppressContentEditableWarning
              onBlur={(event) => updateComponent(component.id, { label: cleanEditedLabel(event.currentTarget.textContent || component.label, component.required, component.requiredForAudit) })}
              style={{ fontWeight: readOnlyPreview ? 300 : componentTextWeight(component, 500), outline: 'none' }}
            >
              {component.label}
              {component.requiredForAudit && <FbRequiredForAudit />}
              {component.required && <span style={{ color: '#d50000', marginLeft: '0.1rem' }}>*</span>}
  ...
  const renderAuthPanel = () => (
    <div style={{ display: 'grid', gridTemplateColumns: '3fr 1fr', height: '100vh', fontFamily: "'Roboto', sans-serif" }}>
      ...
      <aside className="fb-designer-panel" style={{ borderLeft: '0.1rem solid black', padding: '1rem', overflow: 'auto' }}>
        ...
        <hr style={{ border: 'none', borderTop: '0.1rem solid silver', margin: '0 0 0.8rem 0' }} />
  ...
  const renderPropertyGrid = () => {
    ...
    const supportsPlaceholder = ['fbTextInput', 'fbTime', 'fbTextArea', 'fbSmartDropdown'].includes(selectedComponent.type);
    const supportsDefaultValue = questionTypes.includes(selectedComponent.type) && !['fbGroup', 'fbDateHeightWeightBMIRow'].includes(selectedComponent.type);
    ...
    const supportsOptions = selectedComponent.type === 'fbDropdown' || selectedComponent.type === 'fbSmartDropdown';
      ...
      <>
        <table border={1} className="fb-designer-property-grid" style={tableStyle}>
          <tbody>
            ...
            {['fbNumberInputWithUnits', 'fbReadOnly'].includes(selectedComponent.type) && row('Units', (
              <PropertyTextarea
                value={selectedComponent.units || 'units'}
                onChange={(value) => updateComponent(selectedComponent.id, { units: value })}
              />
            ))}
            {selectedComponent.type === 'fbReadOnly' && row('Big label', toggle(selectedComponent.bigLabel, (checked) => updateComponent(selectedComponent.id, { bigLabel: checked })))}
            {selectedComponent.type === 'fbReadOnly' && row('Bold label', toggle(selectedComponent.boldLabel, (checked) => updateComponent(selectedComponent.id, { boldLabel: checked })))}
            {selectedComponent.type === 'fbSubqForOption' && row('Option', parentOptions.length > 0 ? (
              <PropertyDropdown value={selectedComponent.optionValue || ''} onChange={(event) => updateComponent(selectedComponent.id, { optionValue: event.target.value })} style={propertyInputStyle}>
                {parentOptions.map((option) => <option key={option.value} value={option.value}>{option.label || option.value}</option>)}
              </PropertyDropdown>
            ) : (
              <PropertyTextInput value={selectedComponent.optionValue || ''} onChange={(event) => updateComponent(selectedComponent.id, { optionValue: event.target.value })} style={propertyInputStyle} />
            ))}
            ...
            {supportsOptions && selectedComponent.options && row('Options', (
              <PropertyTextarea
                ...
                defaultValue={selectedComponent.options.map((option) => `${option.value}|${option.label}`).join('\n')}
                onBlur={(value) => updateComponent(selectedComponent.id, {
                  options: value.split('\n').filter(Boolean).map((line, index) => {
                    const [rawValue, ...labelParts] = line.split('|');
                    const optionValue = rawValue.trim() || `option${index + 1}`;
  ...
  const runActionAndClose = (runner: () => void) => {
    runner();
    setActionModal(null);
    collapseTaskAccordions();
  };

  const commandButton = (label: string, runner: () => void, danger = false, key = label) => React.createElement(fbcButton, {
    key,
    fullWidth: true,
    danger,
    onClick: () => runActionAndClose(runner),
    children: <span className="fb-designer-action-label">{actionLabel(label)}</span>,
  });

  const actionListItem = (label: string, action: AddAction, parentId = selectedId, type?: DesignerComponentType, danger = false) => React.createElement(fbcButton, {
      key: label,
      fullWidth: true,
      danger,
      onClick: () => {
        if (type) {
          runActionAndClose(() => addComponent(parentId, type, action));
        }
      },
      children: (
        <span className="fb-designer-action-label">{actionLabel(label)}</span>
      ),
    });

  const actionChoiceList = (label: string, action: AddAction, parentId = selectedId, types: DesignerComponentType[] = questionTypes, tableCell = false) => React.createElement(fbcButton, {
    key: label,
    fullWidth: true,
    onClick: () => setActionModal({ kind: 'palette', title: actionLabel(label), action, parentId, types, tableCell } as ActionModalState),
    children: <span className="fb-designer-action-label">{actionLabel(label)}</span>,
  });
  ...
  const renderContextControls = () => {
    ...
    if (selectedTableTarget?.kind === 'header') {
      ...
      controls.push(commandButton('Add column to right', addTableColumnRight, false, 'add-table-column-right'));
      if (canDeleteColumn) {
        controls.push(commandButton('Delete column', deleteTableColumn, true, 'delete-table-column'));
      }
    } else if (selectedTableTarget?.kind === 'row') {
      const table = allComponents(displayedDesign.components).find((component) => component.id === selectedTableTarget.tableId);
      const canDeleteRow = normaliseTableRowCount(table) > 1;
      if (canDeleteRow) {
        controls.push(commandButton('Delete row', deleteTableRow, true, 'delete-table-row'));
      }
    } else if (selectedTableTarget?.kind === 'cell') {
      controls.push(actionChoiceList('Add component here', 'append', selectedTableTarget.tableId, questionTypes, true));
    } else if (selectedTableTarget?.kind === 'separator') {
      controls.push(commandButton('Add column here', addTableColumnAtSeparator, false, 'add-table-column-at-separator'));
      ...
      if (separatorEmptyParent) {
        controls.push(commandButton(separatorDeleteLabel, deleteSelectedSeparatorParent, true, 'delete-empty-separator-parent'));
      }
      ...
      if (selectedComponent.type === 'fbGroup') {
        ...
        controls.push(actionListItem('Add inverse subq', 'subquestion', selectedId, 'fbInverseSubq'));
        ...
        controls.push(actionListItem('Add option subquestion', 'subquestion', selectedId, 'fbSubqForOption'));
      }
    ...
    if (canRemoveEnclosingRowAndCell) {
      controls.push(commandButton('Remove enclosing row and cell components', removeEnclosingRowAndCell, false, 'remove-enclosing-row-and-cell'));
    }
    return (
      <div className="fbc-action-button-stack">
        {controls}
        {!selectedSeparator && !selectedTableTarget && React.createElement(fbcButton, {
          fullWidth: true,
          danger: true,
          onClick: () => setActionModal({ kind: 'delete', title: selectedComponent ? 'Delete component' : 'Delete form' }),
          children: <span className="fb-designer-action-label">Delete</span>,
        })}
        {React.createElement(fbcModal, {
          title: actionModal?.title || '',
          visible: !!actionModal,
          onBack: () => setActionModal(null),
          backLabel: actionModal?.kind === 'delete' ? 'Cancel' : 'Back',
          children: actionModal?.kind === 'palette'
            ? (
              <div className="fbc-action-button-stack">
                {actionModal.types.map((type) => React.createElement(fbcButton, {
                  key: type,
                  fullWidth: true,
                  onClick: () => runActionAndClose(() => {
                    if (actionModal.tableCell) addComponentToSelectedTableCell(type);
                    else addComponent(actionModal.parentId, type, actionModal.action);
                  }),
                  children: <span className="fb-designer-action-label">{typeLabels[type]}</span>,
                }))}
              </div>
            )
            : actionModal?.kind === 'delete'
              ? React.createElement(fbcButton, {
                fullWidth: true,
                danger: true,
                onClick: () => {
                  setActionModal(null);
                  if (selectedComponent) deleteSelected();
                  else if (confirm('Delete form?')) deleteActiveDesign();
                },
                children: selectedComponent ? 'Confirm delete component' : 'Confirm delete form',
              })
              : null,
        })}
      </div>
    ...
    <div style={{ display: 'grid', gridTemplateColumns: publicId ? '1fr' : '3fr 1fr', height: '100vh', fontFamily: "'Roboto', sans-serif", backgroundColor: 'white' }}>
      ...
      {!publicId && React.createElement(fbcPanel, {
        ...
        <>
          ...
          <hr style={{ border: 'none', borderTop: '0.1rem solid silver', margin: '0.5rem 0' }} />
          {!publicId && (
