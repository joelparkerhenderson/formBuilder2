...
{#snippet fieldControl(field: SimpleField)}
  ...
  {:else if field.type === 'msi'}
    <FbMSISelector label={field.label} required={field.required} requiredForAudit={field.requiredForAudit} tooltip={field.tooltip || ''} value={formState[field.key] || ''} coded={formState[`${field.key}_coded`]} onChange={(value, coded) => setCodedValue(field.key, value, coded)} />
