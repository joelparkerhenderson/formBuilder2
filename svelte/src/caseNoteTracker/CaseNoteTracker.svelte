<script lang="ts">
  ...
  onMount(() => {
    ...
    if (loginAs || sessionUuid || inline) {
      login(loginAs || sessionUuid || defaultCntLoginUserUuid, false, patientUuid || initialPatientUuid ? 'locator' : 'home');
    }
  });
  ...
  function openNewRequest() {
    ...
    requestVolumeUuids = selectedVolumeUuid ? [selectedVolumeUuid] : [];
  ...
  function chooseSelectedVolumeForRequest() {
    ...
    toggleRequestVolume(selectedVolumeUuid);
    if (!requestFromLocationUuid) {
      requestFromLocationUuid = store.volumes.find((volume) => volume.uuid === selectedVolumeUuid)?.currentLocationUuid || '';
    }
  ...
  function userLine(item: CntUser) {
    return `${item.surname.toUpperCase()}, ${item.firstNames}, ${item.title}`;
  }

  function userHeader(item?: CntUser) {
    return item ? `${userLine(item)} (NADEX:${item.nadexId}), ${item.role}, ${item.facility}` : '';
  }
  ...
  function volumeSort(a: CntVolume, b: CntVolume) {
    return a.healthBoard.localeCompare(b.healthBoard) || a.locality.localeCompare(b.locality) || a.type.localeCompare(b.type) || a.volumeNumber - b.volumeNumber;
  }
  ...
  function volumeStatus(volume: CntVolume) {
    if (volume.destroyed) return 'Destroyed';
    if (volume.closed) return 'Closed';
    return 'Active';
  }

  function volumeStatusClass(volume: CntVolume) {
    return volume.destroyed || volume.closed ? 'red' : 'green';
  }
      ...
      <div class="login-panel">
        ...
        <div class="login-grid">
          {#each store.users as item}
            <button type="button" class="identity-tile" on:click={() => login(item.uuid)}>
    ...
    <main class="cnt-main">
      ...
      {:else if view === 'home'}
        ...
        <div class="scan-buttons">
          ...
          <button type="button" on:click={() => { const volume = store.volumes[0]; selectedVolumeUuid = volume?.uuid || ''; openPatient(volume?.patientUuid || ''); }}>Simulate volume number scan</button>
    ...
    <footer class="cnt-footer">
      <div class="footer-left">
        {#if view === 'locator'}
          ...
          <FbButton variant="blue" disabled={!selectedVolumeUuid} onClick={chooseSelectedVolumeForRequest}>Request selected</FbButton>
        {:else if view === 'batches'}
...
<style>
  ...
  .volume-row { display: grid; grid-template-columns: 1.2rem 5.5rem 9rem minmax(14rem, 1fr) auto; gap: 0.4rem; align-items: start; }
  ...
  .user-badge { display: block; width: max-content; max-width: 100%; padding: 0 0.2rem; border-radius: 0.2rem; }
  .user-badge:hover, .user-badge:focus { background: #fee715; }
...
{#snippet UserBadge(uuidValue: string)}
  ...
  {#if item}<span class="user-badge" title={`${userLine(item)}\n${item.role}\n${item.facility}\nNADEX id: ${item.nadexId}`}>{item.initials}: {item.nadexId}</span>{/if}
{/snippet}
            ...
            {#each group.volumes as volume}
              {#if interactive}
                <div class="volume-row" class:selected={selectedVolumeUuid === volume.uuid}>
                  <input type="checkbox" disabled={volume.destroyed} checked={selectedVolumeUuid === volume.uuid} on:change={() => selectedVolumeUuid = volume.uuid}>
            ...
            {#each group.volumes as volume}
              <div class="tree-leaf">
                ...
                <div class="received-location">
                  <span class="material-icons received-tick" class:visible={received.has(volume.uuid)}>check_circle</span>
