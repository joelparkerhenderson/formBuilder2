<script lang="ts">
  ...
  export let onChange: (value: string, coded: boolean, nadexId?: string) => void = () => {};
  export let change: (value: string, coded: boolean, nadexId?: string) => void = () => {};
  ...
  const dispatch = createEventDispatcher<{ change: { value: string; coded: boolean; nadexId?: string } }>();
  ...
  function nadexIdFor(result: StaffResult) {
    const direct = result.id || result.code || (result as any).nadexId || (result as any).NADEXId || '';
  ...
  function queueSearch() {
    ...
    dispatch('change', { value: searchText, coded: false, nadexId: '' });
  ...
  function selectResult(result: StaffResult) {
    ...
    onChange(searchText, true, nadexId);
    change(searchText, true, nadexId);
    dispatch('change', { value: searchText, coded: true, nadexId });
  }
  ...
  function clearValue() {
    ...
    onChange('', false, '');
    change('', false, '');
    dispatch('change', { value: '', coded: false, nadexId: '' });
  }
