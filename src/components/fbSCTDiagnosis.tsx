...
export const fbSCTDiagnosis: React.FC<SCTDiagnosisProps> = ({
  ...
  const performSearch = (term?: string, addToHistory: boolean = true) => {
    ...
    if (searchValue.length > 0) {
      ...
      fetch(`https://www.shadesofpale.net/SCTSearch?cmd=findDisorder&st=${encodeURIComponent(searchValue)}&count=30`, {
        method: 'GET'
      })
  ...
  const handleSelect = (disorderName: string) => {
    setJustSelected(true);
    setSearchTerm(disorderName);
    onChange(disorderName);
    setShowDropdown(false);
    setHighlightedIndex(-1);
    setHasSelected(true);
  ...
  const handleMatchClick = (disorderName: string) => {
    performSearch(disorderName, true);
        ...
        {showDropdown && results.length > 0 && fullData && (
          ...
          >
            <div style={{
              ...
              {results.map((disorderName, index) => (
                <div
                  key={index}
                  onMouseDown={(e) => {
                    e.preventDefault();
                    handleMatchClick(disorderName);
                  }}
