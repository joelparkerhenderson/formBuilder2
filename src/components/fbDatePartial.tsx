...
export const fbDatePartial: React.FC<PartialDateProps> = ({
  ...
  const handleBlur = () => {
    ...
    if (inputValue && !date) {
      ...
      if (exactOnly && precision !== 'day') {
        setError('Complete date required');
        return;
      }
  ...
  return (
    <div ref={containerRef} style={{position: 'relative', width: '100%'}}>
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: '0.2rem', width: '100%' }}>
      ...
      {renderRequiredMarkers && (required || requiredForAudit) && (
