<script lang="ts">
  ...
  export let onmouseleave: ((event: MouseEvent) => void) | undefined = undefined;
...
{#if text}
  <svelte:element
    ...
    onclick={onClick}
    onkeydown={onKeydown}
    onmouseenter={(event) => { open = true; onmouseenter?.(event); }}
    onmouseleave={(event) => { open = false; onmouseleave?.(event); }}
    onfocusin={(event) => { open = true; onfocus?.(event); }}
    onfocusout={(event) => { open = false; onblur?.(event); }}
  >
    <slot />
    {#if open}
