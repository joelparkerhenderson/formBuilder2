import { pushWorkspaceState, readWorkspaceState, replaceWorkspaceState } from '$lib/utils/fbNavigationWorkspace';
...
export function formBuilderHomeEntry(): CntNavigationEntry {
  ...
  return { kind: 'url', label: 'formBuilder2 home', url: `${prefix}/` };
}

export function cntHomeEntry(): CntNavigationEntry {
  const prefix = window.location.pathname.startsWith('/formBuilder2/') ? '/formBuilder2' : '';
  return { kind: 'url', label: 'Case note tracker home', url: `${prefix}/cnt` };
}
