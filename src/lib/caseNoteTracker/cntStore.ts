...
function createReviewRequests(volumes: CntVolume[], locations: CntLocation[]): CntRequest[] {
  ...
  const custodianLocations = locations.filter((location) => location.custodianUserUuids.length > 0);
  const activeVolumes = volumes.filter((volume) => volume.status !== 'destroyed');
  ...
  custodianLocations.forEach((targetLocation, locationIndex) => {
    const alternateRequester = users.find((user) => !targetLocation.custodianUserUuids.includes(user.uuid)) || users[0];
    const sourceLocation = locations.find((location) => location.uuid !== targetLocation.uuid && location.acceptsRequests)
      || locations.find((location) => location.uuid !== targetLocation.uuid)
      || targetLocation;
    const custodianUserUuids = targetLocation.custodianUserUuids.length ? targetLocation.custodianUserUuids : [defaultCntLoginUserUuid];
    custodianUserUuids.forEach((custodianUserUuid, custodianIndex) => {
      for (let index = 0; index < 6; index += 1) {
        const outboxVolume = activeVolumes[(locationIndex * 18 + custodianIndex * 6 + index) % activeVolumes.length];
        if (outboxVolume && sourceLocation) {
          requests.push({
            uuid: uuid(`request-review-${targetLocation.uuid}-${custodianUserUuid}-outbox-${index}`),
            volumeUuid: outboxVolume.uuid,
            patientUuid: outboxVolume.patientUuid,
            requestedByUserUuid: custodianUserUuid,
            requiredBy: addDays(2 + index),
            requiredFor: reviewPurposes[index],
            fromLocationUuid: sourceLocation.uuid,
            toLocationUuid: targetLocation.uuid,
            status: 'open',
          });
        }
      }
    ...
    for (let index = 0; index < 6; index += 1) {
      const inboxVolume = activeVolumes[(locationIndex * 18 + 12 + index) % activeVolumes.length];
      if (inboxVolume && sourceLocation && alternateRequester) {
        requests.push({
          uuid: uuid(`request-review-${targetLocation.uuid}-inbox-${index}`),
          volumeUuid: inboxVolume.uuid,
          patientUuid: inboxVolume.patientUuid,
          requestedByUserUuid: alternateRequester.uuid,
          requiredBy: addDays(1 + index),
          requiredFor: reviewPurposes[(index + 2) % reviewPurposes.length],
          fromLocationUuid: sourceLocation.uuid,
          toLocationUuid: targetLocation.uuid,
          status: 'open',
        });
