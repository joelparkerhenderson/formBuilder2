<script lang="ts">
  ...
  let openAbove = false;
  let maxHeight = 300;
  let hoveredResultIndex = -1;
  const codedIconStyle = "color: #008000; font-size: 1.2rem;";
  const warningIconStyle = "color: #fd8a10; font-size: 1.2rem;";
...
<FbQuestion {label}>
  <div class="control-row">
        ...
        <div class="dropdown" class:open-above={openAbove} style={`max-height: ${maxHeight}px`}>
          {#each results as result, index (result.location.uuid)}
            <div
              ...
              style={`background-color: ${highlightedIndex === index || hoveredResultIndex === index ? '#ffffcc' : 'white'};`}
              role="option"
              aria-selected={highlightedIndex === index}
              tabindex="-1"
              onmousedown={() => selectLocation(result.location)}
              onmouseenter={() => { hoveredResultIndex = index; highlightedIndex = index; }}
              onmouseleave={() => hoveredResultIndex = -1}
            >
    ...
    {#if searchTerm && coded}
      <FbToolTip as="span" className="material-icons coded-icon" style={codedIconStyle} text="Coded" tabindex={0}>check_circle_outline</FbToolTip>
    {:else if searchTerm && !coded}
      <FbToolTip as="span" className="material-icons warning-icon" style={warningIconStyle} text="Not coded" tabindex={0}>warning</FbToolTip>
    {/if}
  </div>
</FbQuestion>
