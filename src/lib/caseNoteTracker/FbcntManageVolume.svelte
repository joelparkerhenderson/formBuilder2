<script lang="ts" context="module">
  ...
  export function fbcntVolumeValueFromVolume(volume: CntVolume): FbcntManageVolumeValue {
    ...
    return {
      ...
      dateCreated: toDisplayDate(volume.dateCreated || createdEvent?.datetime.slice(0, 10) || ''),
      dateClosed: toDisplayDate(volume.dateClosed || ''),
      dateDestroyed: toDisplayDate(volume.dateDestroyed || ''),
  ...
  export function fbcntVolumePatchFromValue(value: FbcntManageVolumeValue, fallbackVolume: CntVolume): Partial<CntVolume> {
    ...
    return {
      ...
      dateCreated: toIsoDate(value.dateCreated),
      dateClosed: toIsoDate(value.dateClosed),
      dateDestroyed: toIsoDate(value.dateDestroyed),
  ...
  function toDisplayDate(value: string) {
    const isoDate = formDateToIsoDate(value);
    return isoDate ? formatFormDate(`${isoDate}T00:00:00`) : value;
  }

  function toIsoDate(value: string) {
    return formDateToIsoDate(value) || value.trim() || undefined;
  }
</script>
...
<section class="manage-volume">
  {#if showReadOnlyIdentifiers && value.uuid}
    <div class="summary">
      <FbReadOnly label="Volume UUID" value={value.uuid} />
      {#if currentVolume}<VolumeTagLine {store} volume={currentVolume} />{/if}
    </div>
  {/if}
  <FbHBSelector label="Health board" value={value.healthBoard} options={healthBoardOptions} onChange={(healthBoard) => {
