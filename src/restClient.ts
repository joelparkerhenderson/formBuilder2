...
class RestQueryBuilder {
  ...
  private async executeSelect() {
    ...
    const version = this.getFilter("version")?.value;
    const formUuid = this.getFilter("form_uuid")?.value;
    ...
    switch (this.table) {
      ...
      case "forms_index":
        if (formUuid) return requestJson(`/api/forms-index/${encodeURIComponent(formUuid)}/history`);
        throw new Error("form_uuid filter is required for forms_index history reads");
      ...
      case "cardiology_test_requests":
        ...
        if (typeof version !== "undefined") {
          return requestJson(`/api/forms/${this.table}/${encodeURIComponent(uuid)}/versions/${encodeURIComponent(version)}`);
        }
...
export function createClient(url = "", key = "") {
  ...
  return {
    from: (table: string) => new RestQueryBuilder(table),
