<script lang="ts">
  ...
  $effect(() => {
    sessionStorage.setItem('fb_prev_main_page', `${base}/patient-registry`);
  });
...
<main class="patient-list-page">
  <header class="patient-list-header">
    <a class="patient-list-title" href={`${base}/`}>Patient registry</a>
  </header>

  <section class="patient-list-main" aria-label="Patient registry">
    {#if data.loadError}
      <div class="patient-list-muted" role="alert">{data.loadError}</div>
