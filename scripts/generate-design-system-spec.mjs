...
const patientComponents = sortByName([
  component('fbAddButton', 'src/components/fbAddButton.tsx', 'svelte/src/components/fbAddButton.svelte', 'Inline add control for adding rows/items to form tables and repeatable lists.', '`label`, `onClick`, optional disabled state.', '<button type="button" class="fb-add-button">...', cssBlock('.fb-add-button {\n  border: 0.1rem solid var(--fb-blue);\n  border-radius: 0.4rem;\n  color: var(--fb-blue);\n  background: white;\n  font-family: "Roboto", sans-serif;\n  font-weight: 500;\n}'), 'Native button; accessible name is the visible label.'),
  component('fbBloodPressure', 'src/components/fbBloodPressure.tsx', 'svelte/src/components/fbBloodPressure.svelte', 'Two-part systolic/diastolic blood-pressure input.', '`label`, `systolic`, `diastolic`, `onChange`, `required`, `requiredForAudit`, `valueError`, `tooltip`.', '<span class="fb-blood-pressure-control"><input class="fb-blood-pressure-input">...</span>', cssBlock('.fb-blood-pressure-control {\n  border: 0.1rem solid silver;\n  border-radius: 0.4rem;\n  padding: 0.2rem;\n  display: inline-grid;\n  grid-template-columns: auto auto;\n}\n.fb-blood-pressure-input,\n.fb-blood-pressure-input:focus {\n  border: none !important;\n  box-shadow: none !important;\n  outline: none !important;\n}'), 'Label handled by fbQuestion; inner numeric inputs use standard input semantics.'),
  component('fbBoxedAlert', 'src/components/fbBoxedMessage.tsx', 'svelte/src/components/fbBoxedAlert.svelte', 'Full-width red alert.', '`text` or children.', '<div class="fb-boxed-message" role="alert"><span class="material-icons" aria-hidden="true">error</span>...</div>', cssBlock('.fb-boxed-message {\n  width: 100%;\n  border: 0.2rem solid currentColor;\n  border-radius: 0.4rem;\n  color: var(--fb-red);\n  font-weight: 500;\n}'), '`role="alert"`; icon is decorative.'),
  ...
  component('fbCheck', 'src/components/fbCheck.tsx', 'svelte/src/components/fbCheck.svelte', 'Checkbox option with optional conditional subquestions.', '`label`, `checked`, `onChange`, `required`, `requiredForAudit`, `children`, `tooltip`.', '<label class="fb-radio-checkbox-item"><input type="checkbox">...</label>', cssBlock('.fb-radio-checkbox-item {\n  display: block;\n  padding: 0.1rem;\n}\n.fb-radio-checkbox-item:hover,\n.fb-radio-checkbox-item:focus-within {\n  background: var(--fb-active-lighter-yellow);\n}'), 'Native checkbox; wrapper label provides accessible name.'),
  component('fbDropdown', 'src/components/fbDropdown.tsx', 'svelte/src/components/fbDropdown.svelte', 'Fixed-option selector.', '`label`, `value`, `options`, `onChange`, `required`, `requiredForAudit`, `tooltip`.', '<select class="fb-dropdown">...</select>', cssBlock('.fb-dropdown {\n  border: 0.1rem solid silver;\n  border-radius: 0.4rem;\n  height: 2rem;\n  font-family: "Roboto", sans-serif;\n  font-size: 1rem;\n}'), 'Native select; label is supplied by fbQuestion.'),
  component('fbDateExact', 'src/components/fbDateExact.tsx', 'svelte/src/components/fbDateExact.svelte', 'Complete-date control with picker dropdown.', '`label`, `value`, `onChange`, `required`, `requiredForAudit`, `showRequiredMarkers`, `tooltip`.', '<div class="fb-date-control"><input class="fb-date-control-input">...</div>', cssBlock('.fb-date-control-input {\n  border: 0.1rem solid silver;\n  border-radius: 0.4rem;\n}\n.fb-date-dropdown .button-row {\n  display: flex;\n  width: 100%;\n}\n.fb-date-dropdown .button-row button {\n  flex: 1 1 0;\n}'), 'Input remains keyboard editable; picker buttons are native buttons.'),
  component('fbDateHeightWeightBMIRow', 'src/components/fbDateHeightWeightBMIRow.tsx', 'Svelte follow-up required', 'Date, height, weight, and calculated BMI row.', '`dateRecorded`, `heightCm`, `weightKg`, and matching change handlers.', '<div class="fb-grid-row">Date recorded / Height / Weight / BMI</div>', cssBlock('.fb-bmi-tooltip {\n  cursor: help;\n}\n.fb-grid-row {\n  display: grid;\n  gap: 0.4rem;\n}'), 'Structural row plus labelled native inputs; BMI is read-only display.'),
  ...
  component('fbGroup', 'src/components/fbGroup.tsx', 'svelte/src/components/fbGroup.svelte', 'Labelled group for radio/check options and subquestions.', '`label`, `required`, `requiredForAudit`, `valueError`, `tooltip`, children.', '<div class="fb-radio-checkbox-group-container" role="group">...</div>', cssBlock('.fb-radio-checkbox-group-container {\n  border-radius: 0.4rem;\n  padding: 0.2rem;\n}\n.fb-radio-checkbox-group-container:hover,\n.fb-radio-checkbox-group-container:focus-within {\n  background: var(--fb-active-lighter-yellow);\n}'), '`role="group"` with visible label.'),
  component('fbInverseSubq', 'src/components/fbInverseSubq.tsx', 'Svelte follow-up required', 'Non-indented inverse subquestion for radio/check options.', '`isOpen`, children.', '<div class="fb-subquestion-wrapper inverse">...</div>', cssBlock('.fb-subquestion-wrapper.inverse {\n  padding-left: 0;\n}'), 'Structural grouping only.'),
  ...
  component('fbNotificationTypeGroup', 'src/components/fbNotificationTypeGroup.tsx', 'Svelte follow-up required', 'Notification type radio group with required tooltips.', '`value`, `onChange`, `name`, `label`, `required`.', '<div role="group">Routine notification / Urgent notification</div>', cssBlock('.fb-radio-checkbox-group-container {\n  border-radius: 0.4rem;\n}'), '`role="group"` with native radio children.'),
  component('fbNumberInput', 'src/components/fbNumberInput.tsx', 'svelte/src/components/fbNumberInput.svelte', 'Numeric input without units.', '`label`, `value`, `onChange`, `min`, `max`, `required`, `requiredForAudit`, `valueError`, `tooltip`.', '<input type="number">', cssBlock('input[type="number"].fb-number-input {\n  max-width: 10ch;\n  border: 0.1rem solid silver;\n  border-radius: 0.4rem;\n}'), 'Native number input labelled by fbQuestion.'),
  ...
  component('fbDatePartial', 'src/components/fbDatePartial.tsx', 'svelte/src/components/fbDatePartial.svelte', 'Partial date control accepting exact, month-only, or year-only dates.', '`label`, `value`, `onChange`, `required`, `requiredForAudit`, `showRequiredMarkers`, `tooltip`.', '<div class="fb-date-control"><input class="fb-date-control-input">...</div>', cssBlock('.fb-date-control-input {\n  border: 0.1rem solid silver;\n  border-radius: 0.4rem;\n}\n.fb-date-mode-buttons {\n  display: flex;\n  width: 100%;\n}\n.fb-date-mode-buttons button {\n  flex: 1 1 0;\n}'), 'Input plus native button picker controls.'),
  component('fbRadio', 'src/components/fbRadio.tsx', 'svelte/src/components/fbRadio.svelte', 'Radio option with optional conditional subquestions.', '`label`, `checked`, `name`, `onChange`, `required`, `requiredForAudit`, `children`, `tooltip`.', '<label class="fb-radio-checkbox-item"><input type="radio">...</label>', cssBlock('.fb-radio-checkbox-item {\n  display: block;\n  padding: 0.1rem;\n}\n.fb-radio-checkbox-item:hover,\n.fb-radio-checkbox-item:focus-within {\n  background: var(--fb-active-lighter-yellow);\n}'), 'Native radio; wrapper label provides accessible name.'),
  ...
  component('fbSmartDropdown', 'src/components/fbSmartDropdown.tsx', 'Svelte follow-up required', 'Search-as-you-type dropdown using reusable smart matching.', '`label`, `value`, `onChange`, `options`, `required`, `requiredForAudit`, `placeholder`, children.', '<div class="fb-smart-dropdown"><input role="combobox"><div role="listbox">...</div></div>', cssBlock('.fb-smart-dropdown input {\n  border: 0.1rem solid silver;\n  border-radius: 0.4rem;\n  height: 2rem;\n}\n.fb-smart-dropdown [role="listbox"] {\n  position: absolute;\n  background: white;\n  border: 0.1rem solid silver;\n}'), 'Input uses combobox semantics; result buttons use role option.'),
  component('fbSubqForOption', 'src/components/fbSubqForOption.tsx', 'Svelte follow-up required', 'Indented dropdown option subquestion.', '`optionValue`, `selectedValue`, children.', '<div class="fb-subquestion-wrapper">...</div>', cssBlock('.fb-subquestion-wrapper {\n  padding-left: 1.5rem;\n}'), 'Structural grouping only.'),
  component('fbTable', 'src/components/fbTable.tsx', 'svelte/src/components/fbTable.svelte', 'Form table shell.', '`children`, optional `className`, `style`.', '<table class="fb-table">...</table>', cssBlock('.fb-table {\n  border-collapse: collapse;\n  width: 100%;\n}\n.fb-table th,\n.fb-table td {\n  border-bottom: 0.1rem solid silver;\n  padding: 0.2rem;\n}'), 'Native table element with headers/cells.'),
  component('fbTextArea', 'src/components/fbTextArea.tsx', 'svelte/src/components/fbTextArea.svelte', 'Auto-expanding multi-line text input.', '`label`, `value`, `onChange`, `rows`, `fullWidth`, `required`, `requiredForAudit`, `tooltip`.', '<textarea class="fb-textarea">...</textarea>', cssBlock('.fb-textarea {\n  border: 0.1rem solid silver;\n  border-radius: 0.4rem;\n  resize: none;\n  overflow: hidden;\n  max-width: 37rem;\n}\n.fb-textarea.full-width { max-width: none; width: 100%; }'), 'Native textarea labelled by fbQuestion.'),
  component('fbTextInput', 'src/components/fbTextInput.tsx', 'svelte/src/components/fbTextInput.svelte', 'Standard one-line text input.', '`label`, `value`, `onChange`, `placeholder`, `required`, `requiredForAudit`, `tooltip`.', '<input type="text" class="fb-text-input">', cssBlock('.fb-text-input {\n  border: 0.1rem solid silver;\n  border-radius: 0.4rem;\n  height: 2rem;\n  font-family: "Roboto", sans-serif;\n}'), 'Native text input labelled by fbQuestion.'),
  component('fbTime', 'src/components/fbTime.tsx', 'svelte/src/components/fbTime.svelte', 'Native time input.', '`label`, `value`, `onChange`, `required`, `requiredForAudit`, `tooltip`.', '<input type="time" class="fb-time-input">', cssBlock('.fb-time-input {\n  border: 0.1rem solid silver;\n  border-radius: 0.4rem;\n  height: 2rem;\n}'), 'Native time input labelled by fbQuestion.'),
  component('fbRequiredForAudit', 'src/components/fbRequiredForAudit.tsx', 'svelte/src/components/fbRequiredForAudit.svelte', 'Inline required-for-audit marker.', 'None.', '<span class="fb-required-for-audit">RfA</span>', cssBlock('.fb-required-for-audit {\n  background: var(--fb-orange);\n  color: white;\n  font-size: 1rem;\n  font-weight: 500;\n}'), 'Uses fbToolTip text "Required for audit"; visible label text remains present.'),
...
const otherComponents = sortByName([
  ...
  ['fbBadgeDraft', 'Draft status badge.', 'src/components/fbBadgeDraft.tsx', 'svelte/src/components/fbBadgeDraft.svelte'],
  ['fbBadgeHighlySensitive', 'Highly-sensitive document badge.', 'src/components/fbBadgeHighlySensitive.tsx', 'Svelte follow-up required'],
  ['fbBadgeSuperseded', 'Superseded old-version badge.', 'src/components/fbBadgeSuperseded.tsx', 'Svelte follow-up required'],
  ...
  ['fbModalCancel', 'Cancel confirmation popup.', 'src/components/fbModalCancel.tsx', 'svelte/src/components/fbModalCancel.svelte'],
  ['fbModalDraft', 'Save-as-draft confirmation popup.', 'src/components/fbModalDraft.tsx', 'svelte/src/components/fbModalDraft.svelte'],
  ...
  ['fbModalPassword', 'Second-stage save password popup.', 'src/components/fbModalPassword.tsx', 'svelte/src/components/fbModalPassword.svelte'],
  ...
  ['fbModalSaved', 'Saved confirmation popup.', 'src/components/fbModalSaved.tsx', 'svelte/src/components/fbModalSaved.svelte'],
  ['fbModalSaveError', 'Save-error popup.', 'src/components/fbModalSaveError.tsx', 'svelte/src/components/fbModalSaveError.svelte'],
  ['fbModalSaving', 'Saving progress popup.', 'src/components/fbModalSaving.tsx', 'svelte/src/components/fbModalSaving.svelte'],
...
const icons = sortByName([
  ...
  ['swap_vertical_circle', 'Material Icons', 'Form-table drag handles.'],
...
<body>
  ...
  <ul>
    <li><strong>Addressograph:</strong> any page or form that is about a specific patient shows an <code>fbAddressograph</code> at the top right unless there is a page-specific safety reason to omit it. The addressograph uses an NHS card style and preserves Arial/Helvetica typography rather than global Roboto.</li>
    <li><strong>Date controls:</strong> <code>fbDateExact</code> requires complete dates; <code>fbDatePartial</code> accepts exact, month-only, or year-only dates. Dropdown button rows fill the dropdown width.</li>
    ...
    <li><strong>Save as draft:</strong> draft/final controls and save popups use shared components. Where password confirmation is required, saving includes a second-stage password popup. Password fields are cleared if the user enters a password but leaves the save flow without clicking Save and close.</li>
...
.fb-date-dropdown button {
  ...
  if (exactOnly && !isCompleteDate(value)) return { error: 'Complete date required' };
  if (isCompleteDate(value)) return { kind: 'exact', value: toIsoDate(value) };
  if (!exactOnly && isMonth(value)) return { kind: 'month', value: toIsoMonth(value) };
  if (!exactOnly && /^\\d{4}$/.test(value)) return { kind: 'year', value };
  return { error: 'Date not recognised' };
}

function isCompleteDate(value) {
  return /^\\d{1,2}-[A-Za-z]{3}-\\d{2,4}$/.test(value)
    || /^\\d{1,2}[/.]\\d{1,2}[/.]\\d{2,4}$/.test(value);
}

function isMonth(value) {
  return /^[A-Za-z]{3}-\\d{2,4}$/.test(value)
    || /^\\d{1,2}[/.]\\d{2,4}$/.test(value);
}`)}</code></pre>
...
function resizeTextarea(textarea) {
  ...
  <h3>Password cleanup</h3>
  <p>Authentication and password-confirmation inputs are transient. If a user enters a password but does not complete the Save and close action, the password input and any in-memory pending password value are cleared so the value does not keep the form dirty or persist across workflows.</p>
